...
- Version: product-management-and-order-page-refresh
- Updated by: Codex
- GitHub commits:
  - `877e1f8 Simplify coffee order flow`
  - `345a4f1 Add product management tool`

### Changes

- Reworked the ordering page into a simpler customer flow:
  - Product selection first.
  - Cart/order summary stays visible on desktop.
  - Mobile has a fixed bottom total and checkout button.
  - Checkout confirmation modal remains before final submit.
- Updated brand/page title to `豆之楓企業社`.
- Added a top announcement area and coffee product showcase so customers can immediately understand the page sells coffee beans and related coffee products.
- Updated product price display on add-to-cart controls to use per-package wording, e.g. `$400 / 包`.
- Improved checkout form readability:
  - Input fields now use a stronger border and light background.
  - Added placeholder examples for name, phone, email, 7-11 store, company invoice, contact info, and notes.
- Moved product data from `src/products.js` to `src/products.json` so it can be edited by a management tool.
- Added `商品管理工具.html` for local product management:
  - Add products.
  - Edit product name, category, note, image path, options, and prices.
  - Set status to active, soldout, or hidden.
  - Save back to `src/products.json` in supported browsers.
- Updated `商品上下架說明.md` with the new JSON/tool workflow.

### Backend / API

- Apps Script Web App URL remains:

```txt
https://script.google.com/macros/s/AKfycbzfN28njwcJeZssEQV5HJnZ7Z9Z-dPmIVP0WNLBZNQz7VUG9VewI6hl29-0ivpJ_DiPQA/exec
```

- Order submission still uses `fetch(..., { mode: "no-cors" })`.
- No Apps Script backend file changes were made in this update.
...
- `商品管理工具.html`
- `商品上下架說明.md`
- Removed `src/products.js`

### Verification

- `npm.cmd run lint`: passed.
- `npm.cmd run build`: blocked locally by the Windows Tailwind native dependency loader:
  - `@tailwindcss/oxide-win32-x64-msvc.node`
  - `spawn EPERM`
- `npm.cmd install` and `npm.cmd rebuild @tailwindcss/oxide-win32-x64-msvc` were tried, but the local build issue remained.
- GitHub Pages deployment was triggered by pushing to `main`; deployment build runs on GitHub Actions Ubuntu environment.

### Deployment Notes

- Pushed to GitHub `origin/main`.
- Latest pushed commit: `345a4f1 Add product management tool`.
- GitHub Pages workflow should auto-build and deploy from `.github/workflows/deploy.yml`.
