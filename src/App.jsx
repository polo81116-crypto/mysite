...
const products = [
  ...
  {
    id: "formula-blend-350",
    category: "配方豆專區",
    name: "配方豆 經典平衡",
    roast: "中焙",
    taste: "甜感柔和、口感平衡，適合日常手沖與美式。",
    packageOptions: [{ label: "250g", price: 350 }],
    grindOptions: ["咖啡原豆", "8 磨粉 / 手沖（推薦）", "2.5 磨粉 / 義式"],
    weight: "250g / 配方豆",
    image: "https://images.unsplash.com/photo-1511920170033-f8396924c348?auto=format&fit=crop&w=900&q=80",
  },
  {
    id: "formula-blend-400",
    category: "配方豆專區",
    name: "配方豆 香濃堅果",
    roast: "中深焙",
    taste: "堅果、可可與焦糖感明顯，厚實耐喝。",
    packageOptions: [{ label: "250g", price: 400 }],
    grindOptions: ["咖啡原豆", "8 磨粉 / 手沖（推薦）", "2.5 磨粉 / 義式"],
    weight: "250g / 配方豆",
    image: "https://images.unsplash.com/photo-1459755486867-b55449bb39ff?auto=format&fit=crop&w=900&q=80",
  },
  {
    id: "formula-blend-450",
    category: "配方豆專區",
    name: "配方豆 醇厚焦糖",
    roast: "中深焙",
    taste: "焦糖甜感更明顯，尾韻厚實，適合拿鐵與濃縮。",
    packageOptions: [{ label: "250g", price: 450 }],
    grindOptions: ["咖啡原豆", "8 磨粉 / 手沖（推薦）", "2.5 磨粉 / 義式"],
    weight: "250g / 配方豆",
    image: "https://images.unsplash.com/photo-1447933601403-0c6688de566e?auto=format&fit=crop&w=900&q=80",
  },
  {
    id: "formula-blend-500",
    category: "配方豆專區",
    name: "配方豆 特選黑金",
    roast: "中深焙",
    taste: "厚實、醇順、層次完整，適合重口味與商用出杯。",
    packageOptions: [{ label: "250g", price: 500 }],
    grindOptions: ["咖啡原豆", "8 磨粉 / 手沖（推薦）", "2.5 磨粉 / 義式"],
    weight: "250g / 配方豆",
...
function validateStoreData() {
  ...
  const formulaBlendProductsExist = products.filter((product) => product.category === "配方豆專區").length === 4;
  return Boolean(
    categoryFieldsValid &&
      productFieldsValid &&
      productIdsUnique &&
...
console.assert(products.filter((product) => product.category === "配方豆專區").length === 4, "Formula blend category should contain four products.");
      ...
      <section id="cart" className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <aside className="rounded-[2rem] bg-[#8a603b] p-7 text-white shadow-xl md:p-10">
            ...
            <div className="mt-5 rounded-2xl border border-[#f3c178]/30 bg-[#f3c178]/10 p-4 text-[#fff1df]"><p className="text-sm font-bold text-[#f3c178]">優惠活動</p><ul className="mt-2 space-y-1 text-sm leading-7"><li>• 全館商品 9 折優惠</li><li>• 7-11 賣貨便運費 38 元</li><li>• 折扣後滿 1,000 元享免運</li></ul></div>
      ...
      {showOrderConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-8 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-[#fff8ec] p-6 text-[#2a1a10] shadow-2xl md:p-8">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-[#dccbb2] bg-white p-5"><p className="font-bold text-[#8a603b]">收件資料</p><div className="mt-3 space-y-2 text-sm leading-6 text-[#5c4531]"><p>收件人：{checkoutForm.recipient || "尚未填寫"}</p><p>手機：{checkoutForm.phone || "尚未填寫"}</p><p>Email：{checkoutForm.email || "尚未填寫"}</p><p>取貨門市：{checkoutForm.pickupStore || "尚未選擇"}</p></div></div><div className="rounded-2xl border border-[#dccbb2] bg-white p-5"><p className="font-bold text-[#8a603b]">發票與備註</p><div className="mt-3 space-y-2 text-sm leading-6 text-[#5c4531]"><p>統一編號：{checkoutForm.taxId || "無"}</p><p>公司抬頭：{checkoutForm.companyTitle || "無"}</p><p>備註：{checkoutForm.note || "無"}</p><p>其他資訊：{checkoutForm.socialAccount || "無"}</p></div></div></div>
